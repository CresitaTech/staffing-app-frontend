...
@Component({
  selector: 'app-candidate-review-message',
  templateUrl: './candidate-review-message.component.html',
  styleUrls: ['./candidate-review-message.component.scss'],
  providers: [
    { provide: NgbDateAdapter, useClass: CustomDateAdapter },
    { provide: NgbDateParserFormatter, useClass: CustomDateParserFormatter },
  ]
})
export class CandidateReviewMessageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log('add oninit: ' + this.candidate.submission_date)
  ...
  initialise() {
    ...
    if (this.resumes != undefined){
      let name = (this.resumes.name).split(' ')
      this.candidate.first_name = name[0];
      this.candidate.last_name = (name[1] != undefined)? name[1] : "" + ' ' + (name[2] != undefined)? name[2] : "";
      this.candidate.primary_email = this.resumes.email;
      var numberPhone = (this.resumes.phone).replace(/\D/g, "");;
      var tenDigitPhone = (numberPhone).slice((numberPhone).length - 10); // => "Tabs1"
      this.candidate.primary_phone_number = tenDigitPhone;
      this.candidate.total_experience = this.resumes.total_exp;

      this.candidate.qualification = this.resumes.degree;
      var skillArr = ""
      for(var i=0; i < (this.resumes.skills).length; i++){
        skillArr = skillArr + (this.resumes.skills[i]).replace(/[^\w\s]/gi, "") + ', '
      }
      this.candidate.skills_1 = (skillArr).length > 150 ? (skillArr).substring(0,150) : skillArr
      this.candidate.skills_2 = (skillArr).length > 150 ? (skillArr).substring(150,200) : ""
      this.candidate.additional_qualification = this.resumes.university;

      this.candidate.resume = this.resume_file
      this.candidate.resume_raw_data = this.candidate.first_name + ' ' + this.candidate.last_name
      + ' ' + this.candidate.primary_email  + ' ' + this.candidate.primary_phone_number  + ' ' + this.candidate.total_experience
      + ' ' + this.candidate.qualification  + ' ' + skillArr  + ' ' + this.candidate.additional_qualification
    }
  ...
  isSubmission(){
    ...
    for(var i = 0; i < this.job_descriptions.length; i ++){
      if ((this.job_descriptions[i].status && this.job_descriptions[i].status !== Constants.DEFAULT_STAGE) || (this.job_descriptions[i].stage && this.job_descriptions[i].stage.id !== Constants.DEFAULT_STAGE)) {
