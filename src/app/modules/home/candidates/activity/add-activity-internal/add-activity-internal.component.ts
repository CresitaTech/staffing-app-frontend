...
@Component({
  selector: 'app-add-activity-internal',
  templateUrl: './add-activity-internal.component.html',
  styleUrls: ['./add-activity-internal.component.scss'],
  providers: [
    { provide: NgbDateAdapter, useClass: CustomDateAdapter },
    { provide: NgbDateParserFormatter, useClass: CustomDateParserFormatter },
  ]
})
export class AddActivityInternalComponent implements OnInit {
  ...
  getCandidateById(index): any {
    ...
    const request = [
      ...
      this.service.getCollectionItemByCandidateIdInternal(APIPath.GET_CANDIDATE_JOB_STAGES, index),
  ...
  onSaveJob(item) {
    ...
    this.subscription1$ = this._api.createCollectionItem(APIPath.ADD__INTERNAL_CANDIDATE_JOB_ACTIVITY, item)
      .subscribe((res: any) => {
  ...
  modifyJob(item) {
    ...
    this.job_descriptions.forEach(_ => {
      if (item === _.job_description) {
        ...
        console.log("===========")
        console.log(_)
        this.activity.job_description = _.job_description.id;
        if (_.stage.type === "Internal") {
  ...
  checkEdit(jd){
    console.log("jd");
    console.log(jd.job_description.id);
    if(jd.stage.type !== "Internal"  ){
      var isfoud = false;
      this.job_descriptions.forEach(_ => {
        console.log("job_descriptions");
        console.log(_.job_description.id === jd.job_description.id && _.stage.type === "Internal");
        if(_.job_description.id === jd.job_description.id && _.stage.type === "Internal"){
          isfoud = true;
          return false;
        }
  ...
  onEditJob() {
    ...
    this.subscription1$ = this._api.putCollectionItemById(APIPath.ADD__INTERNAL_CANDIDATE_JOB_ACTIVITY, this.id, object)
      .subscribe((res: any) => {
  ...
  deleteJob(id: string, jd_name: string): void {
    ...
    modalRef.result.then(res => {
      if (res.result) {
        this.subscription1$ = this._api.deleteCollectionItemById(APIPath.ADD__INTERNAL_CANDIDATE_JOB_ACTIVITY, id)
          .subscribe((res: any) => {
