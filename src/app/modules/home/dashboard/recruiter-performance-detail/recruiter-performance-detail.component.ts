...
@Component({
  selector: 'app-recruiter-performance-detail',
  templateUrl: './recruiter-performance-detail.component.html',
  styles: [
    `
    .example-header {
...
export class RecruiterPerformanceDetail extends BaseReportComponent<JobSummaryGraphUnit> implements OnInit {
...
;
  ...
  getSubmissionData(): any {
    ...
    apiParam += `&country=${this.clientCountry}&rid=${this.rid}`;
    ...
    console.log(`/reports/recruiter_calls_performance_summary_by_job_and_recruiter/?${apiParam}`);
    forkJoin([
      this._api.getReportWithApiLink(`/reports/recruiter_calls_performance_summary_by_job_and_recruiter/?${apiParam}`),
    ])
  ...
  downloadCsvFile(): void {
    var apiParam = `date_range=${this.dateRange}`;
    var fileName = 'recruiter_performance_detail_report'
    if (this.dateRange == ReportDateRange.CUSTOM) {
    ...
    apiParam += `&country=${this.clientCountry}&rid=${this.rid}`;
    var csv_api = `/reports/recruiter_calls_performance_summary_by_job_and_recruiter_csv/?${apiParam}`;
  ...
  columnDefs = [
    ...
    { headerName: "Assignment Date", field: "job_date", sortable: true, filter: true,  },
    ...
    { headerName: "Client Interview Reject", field: "RejectedByClient", sortable: true, filter: true },
