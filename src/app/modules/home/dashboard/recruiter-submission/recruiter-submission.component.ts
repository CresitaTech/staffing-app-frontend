...
@Component({
  selector: 'app-recruiter-submission',
  templateUrl: './recruiter-submission.component.html',
  styleUrls: ['./recruiter-submission.component.scss'],
  styles: [
    `
    p {
      font-family: Lato;
...
export class RecruiterSubmissionComponent extends BaseReportComponent<RecruiterGraphUnit> implements OnInit {

  @Input() title: string
  @Input() show_download: boolean;
  @Input() show_tabular: boolean;

  graph_api = '/reports/recruiter_performance_summary_graph/';
  tag = ReportTags.RECRUITER;
  table_api = '/reports/recruiter_performance_summary_table/';
  csv_api = '/reports/recruiter_performance_summary_csv/'
  ...
  ngOnInit(): void {
    ...
    this.getUserForSelection().subscribe(() => {
      ...
      this.filterOnTableRS(this.xAxisFilter, this.xAxisFilterCountry);
      this.filterRecruiterRS(this.xAxisFilter, this.xAxisFilterCountry);
    });
  }
  ...
  filterOnTableRS(recruiter: string, country: string) {
