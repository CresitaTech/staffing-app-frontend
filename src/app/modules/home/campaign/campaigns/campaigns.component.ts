...
export class CampaignsComponent extends CustomFilterModel<EmailList> implements OnInit, OnDestroy, CustomFilterInterface {

  api_path = APIPath.CAMPAIGN;
  ...
  selectHandler() {
    ...
    if (actionValue === "delete") {
      ...
      this.router.navigate(['/home/campaign/campaigns']);
      this.fetchCollectionList();
    }
  ...
  doAction() {
    var modalRef = this.modalService.open(CreateEmailCampaignComponent, _NgbModalOptions);
    modalRef.componentInstance.isCreateList = false;
    modalRef.result.then(res => {
      // if (res.result) {
      //   this.deleteCollectionItem(res.id);
      // }
